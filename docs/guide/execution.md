...
| `fail_fast` | `True` | Stop submitting after the first failure: work already running finishes and is recorded, everything still queued is canceled. `False` runs everything that can still run. |
