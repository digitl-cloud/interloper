...
# ---------------------------------------------------------------------------
# OAuth configuration (ClassVar, not a field type)
# ---------------------------------------------------------------------------
...
class OAuthConfig:
    ...
    Declares that a connection supports OAuth sign-in and specifies
    which provider to use, what scope to request, and how to map the
    token response back to model field names.

    When present on a ``Resource`` subclass as a ``ClassVar``, the
    ``definition()`` method injects an ``x-oauth`` extension into the
    root of the generated JSON Schema so the frontend can render a
    "Sign in with X" button.

    Args:
        provider: OAuth provider key (e.g. ``"amazon"``, ``"google"``).
        auth_url: Authorization endpoint URL for the OAuth provider.
        scope: OAuth scope string to request.
        label: Display label for the provider (e.g. ``"Google"``).
            Defaults to the titlecased provider key.
        icon: Icon identifier for the provider (e.g. ``"devicon:google"``).
    ...
    def __init__(
        ...
        """Initialise an OAuthConfig.

        Args:
            provider: OAuth provider key (e.g. ``"amazon"``).
            auth_url: Authorization endpoint URL.
            scope: OAuth scope string.
            label: Display label (defaults to titlecased provider).
